...
func _ready() -> void:
	if use_resource_loader:
		...
		_resload.connect( "loading_complete", self._on_finished_loading )
...
func _load_level( level_name : String ) -> void:
	...
	if _cur_level:
		...
		_cur_level.disconnect( "player_leaving_level", self._on_player_leaving_level )
		_cur_level = null
	...
	_cur_level.connect( "player_leaving_level", self._on_player_leaving_level )
	...
	for entity_data in _cur_level._map.entity_data[level_name].entities:
		...
		entity._entity_initialize( entity_data.parameters )
